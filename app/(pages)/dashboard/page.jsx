  ...
  return (
    <div className='p-4 sm:py-6 px-10'>
      ...
      <div className="welcome bg-cyan-100 p-2 rounded my-4 shadow shadow-gray-300">
        ...
        {upcomingMeetings && upcomingMeetings.length > 0 ? <div>
          <h3 className='text-gray-700 mt-4'>Your upcoming meetings</h3>
          {upcomingMeetings.map(m => (<p className='text-sm text-gray-600 mt-2 bg-gray-100/50 p-2 rounded-lg' key={m.id}>- <b>{m?.event?.title}</b> with {m?.name} on {format(new Date(m?.startTime), "MMM d, yyyy")} at {format(new Date(m?.endTime), "h:mm a")}</p>))}
      ...
      <div className="link bg-cyan-100 p-2 py-4 rounded my-4 shadow shadow-gray-300">
        <div className='flex gap-4 items-center'>
          ...
          <Copy
            className='mb-1 hover:bg-gray-200/50 rounded p-1 cursor-pointer duration-100'
            onClick={() => {
              navigator.clipboard.writeText(`${window?.location.origin}/${user?.username}/`);
              toast.success("Link copied to clipbaord!")
            }}
            size={"24px"} color='darkblue' />
        </div>
